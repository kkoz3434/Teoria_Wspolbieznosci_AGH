...
Conductor.prototype.release = function (i) {
    var conductor = this;
    conductor.tableTaken[i] = 0;
}

Conductor.prototype.tryAquire = function () {
    var sitting = 0;
    var conductor = this;
    for (let i = 0; i < conductor.N; i++) {
        sitting += conductor.tableTaken[i];
    }
    if (sitting === N - 1) {
        return false;
    }
...
for (var i = 0; i < N; i++) {
    ...
    ///philosophers[i].startConductor(conductor, M);
    philosophers[i].acquireTwoForksAtOnce(M)
}
...
setTimeout(function () {
    ...
    for (let i = 0; i < N; i++) {
        ...
        for (let j = 0; j < philosophers[i].times.length; j++) {
            suma+= philosophers[i].times[j];
        }
        console.log(i + " " + (suma/N));
    }
}, 10000);
